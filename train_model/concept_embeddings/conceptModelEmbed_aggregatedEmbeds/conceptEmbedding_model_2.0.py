...
class ConceptEmbedModel(nn.Module):
    def __init__(self, a, embedding_dim, pretrained_weight, ancestor_hierarchy, concepts):
        ...
        self.H = Variable(torch.randn(len(self.concepts), 100, dtype=torch.double) * 1e-9, requires_grad=False)
    
    def forward(self, x, y):
        
        local_H = torch.zeros(len(self.concepts), 100, dtype=torch.double, requires_grad=False)
...
def train_model(opt, pretrained_weight, word2idx, idx2word, matrix_hierarchy, concepts):
    ...
    net = ConceptEmbedModel(len(concepts), 100, pretrained_weight, matrix_hierarchy, concepts)
    net = net.to(device)
    try:
...
def setup_model(opt):
    ...
    concepts = word2idx
    pretrained_weight = np.random.uniform(low=-1.0e-9, high=1.0e-9, size=(len(concepts), 100))
    #pretrained_weight = np.zeros((len(concepts), 100))

...
def get_abbrs():
    ...
    m = open("cleaned_allacronyms_dict_20190402_NEW.pickle", 'rb')
...
def load_data(abbr_samples, abbr):
    ...
    for i in abbr_samples:
        ...
        sample = AbbrRep()
        sample.label = label
        if sample.label not in name2meta[abbr] and sample.label != abbr:
            print(sample.label)
            continue
...
def create_embed(opt):
    ...
    for j in range(start, end):
        ...
        #dest_dir = "/Volumes/terminator/hpf/abbr_dataset_pickle_w5_20190319/"
        root = "w" + str(opt.window) + "_ns" + str(opt.ns) + "_"
        if opt.g:
            root += "g_"
        root += "20190408"
        dest_dir = "/Volumes/terminator/hpf/abbr_dataset_pickle_" + root + '/'
        print(dest_dir)
        if not os.path.exists(dest_dir):
            os.mkdir(dest_dir)
        outputfile = abbr + "_" + root + ".pickle"
        print(os.path.join(dest_dir, outputfile))

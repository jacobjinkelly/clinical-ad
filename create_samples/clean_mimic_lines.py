...
def createSentences(input, window, original_word, category):
    ...
    try:
        ...
        text = split_sent[2]
        split_on_sense = text.split(abbr_meaning)
        # doc = ''
        #
        ...
        try:
            ...
            if category == 2 and original_word != "":
                abbr_meaning = original_word
            output = str(abbr_meaning) + "|" + str(doc_whole) + "|" + str(doc_left) + "|" + str(' '.join(local_sentence_left))+ "|" + str(' '.join(local_sentence_right))
